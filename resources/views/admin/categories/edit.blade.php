...
@section('admin_content')
    <div style="margin-bottom: 30px;">
        ...
        <h2 style="color: var(--primary); font-weight: 800;">Edit Category: {{ $category->name }}</h2>
        <p style="color: #64748b; font-size: 14px;">Modify the classification details for this equipment group.</p>
    </div>

    <div class="admin-card" style="max-width: 800px;">
        <form action="/admin/categories/{{ $category->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            <!-- We use POST with /admin/categories/{id} for simplicity since I registered it as POST in web.php -->
            <div style="display: grid; grid-template-columns: 1fr; gap: 25px;">
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="{{ $category->name }}" required style="width: 100%; padding: 15px 20px; border: 2px solid #eef2f6; border-radius: 12px; font-size: 15px;">
                </div>
                ...
                <div class="form-group">
                    ...
                    @if($category->image)
                    <div style="margin-bottom: 20px; display: flex; align-items: center; gap: 20px; background: #f8fafc; padding: 20px; border-radius: 15px; border: 1px solid #eef2f6;">
                        <img src="{{ asset('storage/' . $category->image) }}" style="width: 100px; height: 100px; border-radius: 12px; object-fit: cover;">
                        <div>
                            <p style="font-weight: 700; color: var(--primary); margin-bottom: 5px;">Current Image</p>
                            <p style="font-size: 12px; color: #64748b;">Upload a new one below to replace this.</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div style="border: 2px dashed #e2e8f0; padding: 40px; border-radius: 20px; text-align: center; background: #fafbfc;">
                        ...
                        <p style="font-size: 14px; color: #64748b; margin-bottom: 15px;">Drag and drop or click to replace image</p>
                ...
                <div style="padding-top: 20px; border-top: 1px solid #f1f5f9; display: flex; gap: 15px;">
                    <button type="submit" class="btn btn-primary" style="padding: 15px 40px; border-radius: 12px; font-weight: 800;">UPDATE CATEGORY</button>
